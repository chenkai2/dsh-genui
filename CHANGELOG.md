...
### 新增
- 公式支持补齐四种分隔符、强调内嵌套、矩阵/分段/多行推导；标题、步骤、时间线、选项及显示标签统一走公式渲染，交互原值保持不变（#162）。
- 文本、列表、表格文本列、键值与提示框支持 `$...$` 行内公式和 `$$...$$` 独立公式，复用宿主公式渲染与字体（#162）。

### 修复
- 被丢弃组件的校验结果保留具体字段错误，避免模型只收到泛化提示（#163）；字段速查补全由 #164 提供。
- DOM 围栏识别忽略语言标签两端空白，拒绝疑似消息容器时逐块记录结构原因，仍保留原文（#158）。
- 常驻提示与技能明确要求正文和界面文案跟随用户语言，不因中文示例切换语言（#165）。
- **浅色主题卡片发灰（issue #159）**：0.10.0 为了让浅色卡片"看得见"，把卡面改成 10% 的 label 叠加（页面 255 → 卡面 `#e7e7e8`），卡片比页面更暗，观感是凹陷/禁用而不是抬升。浅色卡面改回宿主层色（浅色下即 `#ffffff`），抬升继续由 1px 描边 + 分层阴影承担；深色主题与其它组件未改动。新增真实浏览器断言钉住方向（浅色卡面不得比页面暗、深色卡面必须比页面亮），并加 `E2E_PLUGIN_SPEC` 以便对已发布版本出"改前"基线。
